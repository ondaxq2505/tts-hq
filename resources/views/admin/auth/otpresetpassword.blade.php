...
@section('content')
    <div style="max-width: 400px; margin: 0 auto; padding: 40px; border: 1px solid #ddd; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.05);">
        <h2 style="text-align: center; margin-bottom: 20px;"> Xác minh OTP quên mật khẩu</h2>
        ...
        <form method="POST" action="{{ route('admin.otp.verifyresetpassword') }}">
            @csrf
        ...
        <form method="POST" action="{{ route('admin.otp.resend_reset_password') }}" style="margin-top: 15px; text-align: center;">
            @csrf
            <button type="submit"
